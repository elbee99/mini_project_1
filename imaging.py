...
#     for j in range(123,135):
#         # this does look like a mess, but only takes
#         # about 0.5 ms per iteration, ~1% of total time
...
#         thickness_arr.append(cuo_layer.thick.value)
#         stderr_arr.append(cuo_layer.thick.stderr)
        
# print(thickness_arr[0:5],stderr_arr[0:5])
# print("Time taken for 180 fits= {}".format(time.time()-start))
...
if __name__ == '__main__':
    ...
    with open("thickness_arr_manager.pkl",'wb') as f:
        pickle.dump(thickness_arr,f) 

    with open("thickness_arr_png_manager.pkl",'wb') as f:
        pickle.dump(thickness_png_array,f) 

    with open("stderr_arr_manager.pkl",'wb') as f:
        pickle.dump(stderr_arr,f) 
    with open("stderr_arr_png_manager.pkl",'wb') as f:
...
"""lambda_arr_flat = lambda_arr.reshape(-1, 1)
aoi_arr_flat = aoi_arr.reshape(-1, 1)
psi_arr_flat = psi_arr.reshape(-1, 1)
delta_arr_flat = delta_arr.reshape(-1, 1)
data = np.concatenate((lambda_arr_flat, aoi_arr_flat, psi_arr_flat, delta_arr_flat), axis=1)

fitter = CurveFitter(ObjectiveSE(model,data))
fitter.fit()"""

"""
        array_list = []
    sample_image_path = os.path.join(dir,df['Delta'][0])
    image_width = get_im_width(sample_image_path)
    image_height = get_im_height(sample_image_path)
    # print(sample_image_path,image_height,image_width)
    for i in range(image_width*image_height):
        temp_df = df.copy(deep=True) #to preserve original df
        #iterate over png files pointed to in Delta column
        list_delta = []
        list_psi = []
        for png_path in temp_df['Delta']:
            #convert png to np.ndarray
            png_array = imread(os.path.join(dir,png_path))
            #replace the filepath with the ith value in the flattened
            #png image 
            list_delta.append(float(png_array.reshape(image_height*image_width,1)[i]))

        #iterate over png files pointed to in Psi column
        for png_path in temp_df['Psi']:
            #convert png to np.ndarray
            png_array = imread(os.path.join(dir,png_path))
            #replace the filepath with the ith value in the flattened
            #png image 
            list_psi.append(float(png_array.reshape(image_height*image_width,1)[i]))
        temp_df['Delta'] = pd.Series(list_delta)
        temp_df['Psi'] = pd.Series(list_psi)
        print(i)
        array_list.append(temp_df)
 """
